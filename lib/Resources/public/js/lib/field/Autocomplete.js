...
var scrollKeys = [33, 34, 35, 36, 38, 40],
    ...
    {
        this.extend(this, ag.u.tpl("agitadmin-autocomplete", ".autocomplete"));
    ...
    typingLoop = function()
    {
        ...
        if (searchTerm.length >= this.minLength && searchTerm !== this.lastSearchTerm)
        {
            ...
            ag.s.api.doCall(this.endpoint, searchTerm, result => {
                this.list.empty();
                this.rows.length = 0;
                ...
                {
                    this.list.html(ag.u.tpl("agitadmin-autocomplete", ".empty"));
                }
...
autocomplete.formatter = entity => ag.u.out(entity ? entity.name || entity.id : "");

autocomplete.rowGenerator = function(entity)
{
    var row = ag.u.tpl("agitadmin-autocomplete", "li.item");

    row.append($("<span>").text(ag.u.out(entity.name)));
    row.append($("<span class='extra mono'>").text("#" + entity.id));
