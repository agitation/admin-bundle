...
var
    ...
    duplicate = function(link, item, changedFields, reload)
    {
        ...
        if (reload)
            ag.srv("api").doCall(
                item.getName() + ".get",
                item.id,
                realItem => duplicateCall.call(this, link, realItem, changedFields)
            );
        else
            duplicateCall.call(this, link, item, changedFields);
    },

    duplicateCall = function(link, item, changedFields)
    {
...
entityListTable._filters =
{
    ...
    datetime : function(item, fieldName)
    {
        ...
        return $("<span class='datetime'></span>").text(ag.ui.tool.date.format(date, ag.intl.x("date format", "Y-m-d H:i")));
    },
...
entityListTable._actions =
{
            ...
            $link.click(() => {
                ...
                if (!hasFields)
                {
                    modal = new ag.ui.elem.ConfirmModal(
                        ...
                        duplicate.bind(this, $link, item, null, params.reload)
                    );
                }
                else
                {
                    ...
                    modal = new ag.admin.CloneModal(fields, () => duplicate($link, item, modal.getValues(), params.reload));
                }
    ...
    remove : {
        ...
        generate : function($link, item) {
            ...
            item.deleted !== undefined && $link.find("span").text(ag.intl.t("delete permanently"));
